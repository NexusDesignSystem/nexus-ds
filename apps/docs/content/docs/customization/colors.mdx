---
title: Colors
description: Learn how to configure and customize colors with Nexus Design System.
---

# Colors

Nexus Design System's plugin enables you to personalize the semantic colors of the theme such as `primary`,
...
Nexus Design System offers a default color palette right out of the box, perfect for when you're 
...
> Semantic colors should be placed inside the `nexusui` plugin options, not inside the TailwindCSS theme object.
...
```js
import {commonColors, semanticColors} from "@nexus-ds/theme";
...
Nexus Design System creates CSS variables using the format `--prefix-colorname-shade` for each semantic color. By
...
/* With default prefix */
.my-component {
  background-color: var(--nexusui-primary-500);
}
/*  With custom prefix */
.my-component {
