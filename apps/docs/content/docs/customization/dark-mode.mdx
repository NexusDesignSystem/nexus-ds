---
title: Dark mode
description: Nexus Design System comes with two palette modes light (the default) and dark.
---

# Dark mode

As we mentioned before in the [Theme](/docs/customization/theme) section Nexus Design System comes with two default
...
Component with the `NexusUIProvider` and the `next-themes` provider components.
...
> Note: We're using the `class` attribute to switch between themes, this is because Nexus Design System uses the `className` attribute.
