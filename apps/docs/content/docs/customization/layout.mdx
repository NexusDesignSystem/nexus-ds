---
title: Layout
description: Nexus Design System offers a set of layout options to customize the theme.
---

# Layout

Nexus Design System's plugin provides a variety of layout customization options. Alter spacing units, 
...
`spacingUnit` token (default 4px), Nexus Design System auto-generates the following units:
...
Nexus Design System units behave like [Tailwind CSS spacing](https://tailwindcss.com/docs/customizing-spacing#default-spacing-scale) units. You can use them in the `margin`, `padding`,
...
Nexus Design System creates CSS variables using the format `--prefix-prop-name-scale` for each layout token. By
default the prefix is `nexusui`, but you can change it with the `prefix` option.
...
/* With default prefix */
.my-button {
  padding: var(--nexusui-spacing-unit-4);
  font-size: var(--nexusui-font-size-small);
  line-height: var(--nexusui-line-height-small);
  border-radius: var(--nexusui-radius-medium);
}
/*  With custom prefix */
.my-component {
