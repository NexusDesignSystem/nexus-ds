...
import {Tabs, Tab, Card, CardBody, Image, Button, RadioGroup, Radio} from "@nexus-ds/react";
...
import {NexusLogo, CodeWindow} from "@/components";
...
const themesTabs = (isMobile: boolean) => [
  {
    id: "nexusui",
    title: () => <p className="group-data-[selected=true]:text-primary">Nexus Design System</p>,
    icon: () => (
      <NexusLogo
        small
        ...
        <CardBody className="relative flex-col md:flex-row md:items-center gap-4 md:gap-9 overflow-visible">
          ...
          <div className={slots.contentWrapper()}>
            <div className="relative flex flex-wrap items-baseline">
              <h1 className={slots.title()}>Sataru Gojo Wallpaper</h1>
              <p className={slots.description()}>Sleek, Elegant, Neon </p>
